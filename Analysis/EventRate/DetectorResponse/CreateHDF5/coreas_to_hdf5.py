    ...
    for key, value in items:
        if key in ["CoreCoordinateNorth", "CoreCoordinateWest", "CoreCoordinateVertical",
            ...
            if key not in ["EventNumber", "RunNumber", "GPSSecs", "GPSNanoSecs", "PrimaryParticleType"]:
                f_h5_reas.attrs[key] = np.float64(value)
            else:
...
def read_antenna_data(hdf5_file, list_file, antenna_folder):
    ...
    for line in lines:
        ...
        data_set = observers.create_dataset(antenna_label, data.shape, dtype=np.float64)
        data_set[...] = data

        data_set.attrs['position'] = np.array(antenna_position, dtype=np.float64)
